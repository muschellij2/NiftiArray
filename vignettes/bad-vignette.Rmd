---
title: "NiftiArray: An R Package to Implement Fast Random Access of NIfTI Objects"
...
R is not well suited for big datasets. NIfTI images, depending on the image dimension and voxel size, can be quite large when loaded and memory is a concern. As sample size increases it becomes difficult to perform simple operations across subjects. For example, calculating the mean image across 800 subjects is computationally intense since not all 800 subjects can be loaded into memory at once. Therefore, there is a need for alternative approaches. 
...
Current methods were not designed for use explicitely with 3D array or images but support. So we extended and built something to seamlessly work with images....

DelayedArray tools...



The `NiftiArray` package allows for fast [random access](https://en.wikipedia.org/wiki/Random_access) of imaging data in NIfTI format and supports [`DelayedArray` operations](https://bioconductor.org/packages/release/bioc/html/DelayedArray.html). The pacakge establishes the `NiftiArray` class, a convenient and memory-efficient array-like container for on-disk representation of NIfTI image(s). The `NiftiArray` class is an extension of the [`HDF5Array` class](https://bioconductor.org/packages/release/bioc/html/HDF5Array.html) and converts NIfTI objects on disk to HDF5 files which allow for block processing and memory-efficient reprresentations in R.

This package allows users to save NIfTI objects as HDF5 files while conserving the array structure and NIfTI header. Once the data are stored as an HDF5 they can be represented as a `NIfTIArray` object. This object all
